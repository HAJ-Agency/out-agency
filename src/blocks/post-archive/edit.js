...
const edit = (props) => {
   ...
   React.useEffect(() => {
      ...
      if (!mainClassName) {
         setAttributes({
            mainClassName: `post-archive swiper block-editor-block-list__block wp-block ${blockProps.className} `,
         });
      }
   }, []);
   ...
   return (
      <>
         <InspectorControls>
            <PanelBody title="Post Arkiv Inställningar">
               <SelectControl
                  label="Post Typ"
                  value={postType}
                  options={postTypes}
